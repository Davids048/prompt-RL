...
DiffusionNFT/
  Vendored DiffusionNFT source tree, added as ordinary root-tracked source by
  explicit user request. Its one-time upstream provenance is recorded only in
  `DiffusionNFT/PROVENANCE.md`; do not edit or append to that file unless the
  user explicitly allows it. Generated reproduction outputs, venvs, caches, and
  reward checkpoints under this tree remain ignored.
...
Root versioning policy is intentionally narrow. Track the repo README, code, reusable prompt data, memory, research Markdown/text notes, `externals.lock.md`, `patches/reward-server/*.patch`, `patches/slime/*.patch`, and the vendored `DiffusionNFT/` source exception. Do not track `plan.md`, `reports/`, generated outputs, local environments, caches, raw artifacts, or nested third-party repo checkouts.
